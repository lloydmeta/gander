...
/**
  ...
  * Time: 2:50 PM
  */
abstract class PublishDateExtractor extends Extractor[Option[DateTime]] {
...
object DefaultPublishDateExtractor extends PublishDateExtractor {

  private val dateParser = org.joda.time.format.ISODateTimeFormat.dateTimeParser

  def extract(rootElement: Element): Option[DateTime] = {
    // Try to retrieve publish time from open graph data
    for (el <- rootElement.select("meta[property=article:published_time]").asScala)
      return Try(dateParser.parseDateTime(el.attr("content"))).toOption
    None
  }
