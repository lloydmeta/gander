...
object PerformanceTester {
  ...
  def main(args: Array[String]) {
    ...
    val html =
      FileHelper.loadResourceFile(TestUtils.staticHtmlDir + "scribd1.txt", Gander.getClass)
    val url =
      "http://www.scribd.com/doc/52584146/Microfinance-and-Poverty-Reduction?in_collection=2987942"
    ...
    for (i <- 0 to 100) {
      goose.extractArticleData(url, html)
    }
    clock.stop()
