...
class TextExtractions {
  ...
  def cnn1() {
    ...
    TestUtils
      .runArticleAssertions(article = article, expectedTitle = title, expectedStart = content)
  }
  ...
  def foxNews() {
    ...
    val content =
      "Russia's announcement that it will help Iran get nuclear fuel is raising questions"
    val article = TestUtils.getArticle(url = url, rawHTML = html)
  ...
  def testHuffingtonPost() {
    ...
    val keywords =
      "federal, reserve's, low, rate, policy, is, a, 'dangerous, gamble,', says, top, central, bank, official, business"
    val description =
  ...
  def wiredPubDate() {
    ...
    implicit val config =
      Configuration.Default.copy(publishDateExtractor = new PublishDateExtractor() {
        @Override
        def extract(rootElement: Element): Option[DateTime] = {
          ...
          if (metaDisplayDate.hasAttr("content")) {
            ...
            return Some(fmt.parseDateTime(dateStr))
          }
          None
        }
      })
    ...
    assertEquals("Publish date should equal: \"2010-08-18\"",
                 expectedDateString,
                 article.publishDate.get.toString(fmt))
    System.out.println("Publish Date Extracted: " + article.publishDate.get.toString(fmt))
  ...
  def msn1() {
    ...
    val url: String =
      "http://lifestyle.msn.com/your-life/your-money-today/article.aspx?cp-documentid=31244150"
    val article = TestUtils.getArticle(url, html)
    TestUtils.runArticleAssertions(article = article, expectedStart = expected)
  }
  ...
  def businessinsider1() {
    ...
    TestUtils.runArticleAssertions(article = article,
                                   expectedStart =
                                     "As everyone in the world was transfixed on the Fed")
  ...
  def issue24() {
    ...
    assertEquals("The beginning of the article text was not as expected!",
                 expected,
                 article.cleanedArticleText.get)
  }
